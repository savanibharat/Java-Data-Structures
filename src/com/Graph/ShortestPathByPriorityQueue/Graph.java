...
public class Graph {
	...
	/** The pq. */
	public PriorityQueue<Vertex> pq = new PriorityQueue<Vertex>(200,
			new Vertex());
	...
	public void dijkstra(String init) {
		...
		while (handled < vertices.size()) {
			...
			if (current.known == false) {
				...
				System.out.format("Cost: %-8d", (int) current.distance);
				printPath(current);
				System.out.println();
	...
	public void compAdjEdges(Vertex s, int w) {
		...
		int origDist;
		/*
		 * Each adjacent edge to the source Vertex, (if it has not yet been
		 * handled) has a weight which is added to the current pathWeight. If
		 * this pathWeight is smaller than the one associated with the given
		 * edge's vertex (destination.getDistance()) then the vertex's distance
		 * is updated and this path is added to the priority queue
		 */
